...
instance Monad FV where 
  return x = FV (\i -> (i, x))
  c1 >>= c2 = FV $ \i -> let (i', x) = runFV c1 i 
                         in runFV (c2 x) i' 
                            
getFreshVar :: FV TypeTemp
getFreshVar = FV $ \i -> (i+1, TT_var i)

getFreshVarT = FVT $ \i -> return $ runFV getFreshVar i 

execFV :: FV t -> t
execFV c = snd $ runFV c 0 

newtype FVT m t = FVT {runFVT :: Int -> m (Int, t)}

instance Monad m => Monad (FVT m) where 
  return x = FVT $ \i -> return (i, x) 
  c >>= f = FVT $ \i -> do (i', x) <- runFVT c i 
                           runFVT (f x) i'

instance MonadTrans FVT where 
  lift c = FVT $ \i -> c >>= \x -> return (i, x) 
...
typeInferTemp :: TempContext -> PcfTerm -> TypeTemp -> FVT Maybe TypeAssign
typeInferTemp ctx (TM_var str) t =
  lift $ tempOfVar str ctx >>= \t' -> typeUnif [(t', t)] 
typeInferTemp ctx (TM_num _) t = 
  lift $ typeUnif [(TT_nat, t)] 
typeInferTemp ctx (TM_op e1 e2) t = 
  do as <- lift $ typeUnif [(t, TT_nat)] 
...
typeInferTemp ctx (TM_pair e1 e2) t = 
  do fvar1 <- getFreshVarT   
     fvar2 <- getFreshVarT
     as <- lift $ typeUnif [(t, TT_prod fvar1 fvar2)] 
     let ctx1 = assignContext ctx as 
...
typeInferTemp ctx (TM_injfst e) t = 
  do fvar1 <- getFreshVarT
     fvar2 <- getFreshVarT
     as <- lift $ typeUnif [(TT_sum fvar1 fvar2, t)] 
...
typeInferTemp ctx (TM_abs str e) t =
  do fvar1 <- getFreshVarT 
     fvar2 <- getFreshVarT
     as <- lift $ typeUnif [(TT_arrow fvar1 fvar2, t)] 
