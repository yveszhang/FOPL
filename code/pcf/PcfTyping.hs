...
typeInferTemp :: Int -> TempContext -> PcfTerm -> TypeTemp -> Maybe (TypeAssign, Int)
typeInferTemp n ctx (TM_var str) t =
  case tempOfVar str ctx 
  of Nothing -> Nothing 
     Just t' -> case typeUnif [(t', t)] 
                of Nothing -> Nothing 
                   Just as -> Just (as, n)
typeInferTemp n ctx (TM_num _) t = 
  case typeUnif [(TT_nat, t)] of Nothing -> Nothing 
                                 Just as -> Just (as, n)
typeInferTemp n ctx (TM_op e1 e2) t = 
